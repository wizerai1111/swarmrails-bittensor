...
serve(async (req) => {
            ...
            if (statusRes?.ok) {
              ...
              if (falStatus.status === 'COMPLETED' && falStatus.response_url) {
                ...
                if (resultRes?.ok) {
                  ...
                  const mediaUrl = result.video?.url || result.output?.video?.url
                    || result.mesh_url || result.model_mesh?.url || result.model?.url
  ...
  if (req.method === 'POST') {
    try {
      const { prompt, netuid, agent_id } = JSON.parse(await req.text())
      ...
      // 6. Confirm USDC reached the recipient wallet
      ...
      const validPayment = (receipt.logs ?? []).some((log: any) =>
